...
User-defined HTTP endpoint health and security-header checker.

School Purpose Only.

## Overview

Endpointcheck is a polished educational cybersecurity utility by LayerByte. It focuses on one practical defensive concept and keeps the implementation small enough for students to read, run, and understand.

Basic endpoint health and response headers reveal useful information for defensive web administration.

## Highlights

- Checks response availability
- Reviews selected security headers
- Uses safe timeouts
...
- User-supplied HTTP or HTTPS URLs
- Expected status or header checks where configured
...
```bash
git clone https://github.com/LayerByte/endpointcheck.git
cd endpointcheck
```
...
A concise endpoint report covering status, timing, and selected headers.
